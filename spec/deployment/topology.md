...
**Companions:** `spec/deployment/environments.md` (environment contract), `spec/deployment/configuration.md` (configuration and secret boundary contract), `spec/deployment/packaging.md` (runtime packaging contract), `spec/deployment/ci-cd.md` (CI/CD and promotion contract, DEP-006), `spec/deployment/observability.md` (observability/resilience/DR contract, DEP-007), `spec/deployment/production-readiness.md` (production readiness proof contract, DEP-008), `deploy/contracts/components.json` (machine-readable component registry), `scripts/validate_deployment.py` (static/configuration verification gate)
...
> **Governed contract change — DEP-006** (same clause): the CI/CD and
> promotion contract landed as **repository-local deployment-layer
> automation** — `spec/deployment/ci-cd.md` is the contract document. The
> gate runner (`scripts/run_ci_gates.mjs`) executes the full verification
> battery (governance, deployment-contract, durable-contract, typecheck,
> every `scripts/test_*.mjs` harness by glob, build) and emits a
> machine-parseable JSON gate table; the promotion tool
> (`scripts/promote.mjs`) freezes exact immutable revisions (commit SHA,
> tree digest, tracked-file content digest + the fresh gate table) as
> append-only JSONL promotion records in
> `deploy/promotions/promotion-records.jsonl`, gates migrations against a
> THROWAWAY database copy only, re-verifies records in a temp worktree at
> the recorded SHA, and EMITS rollback plans honoring R1–R5 below
> (UNKNOWN never retried; post-effect cases listed, not executed).
> The GitHub Actions workflows (`.github/workflows/ci.yml`,
> `promotion.yml`) invoke exactly these repo-local scripts — no gate
> exists that only CI can run. No present-set change (all components
> unchanged): the registry gained the top-level `ci_cd` contract object,
> and all four surfaces — components.json, this document,
> spec/deployment/configuration.md and the validator — were updated
> together in that one work item. Production deployment binding remains
> FUTURE-WORK: every `production_gate` stays authoritative and the
> promotion tool refuses a resolved `PAYSWAP_ENV=production` context
> outright (unverified production promotion is forbidden).
...
- **DEP-007** landed the observability, resilience and disaster-recovery contract (`spec/deployment/observability.md`) as in-process library families — no present-set change (all eleven components unchanged): `src/lib/observability/` (the nine-domain telemetry taxonomy `command | queue | execution | unknown | reconciliation | clearing-netting | settlement-finality | incident-recovery | deployment` as a closed frozen tuple with an exhaustive Record; the read-only `TelemetrySnapshot` collector over durable_jobs/durable_events/the rail activity surface/the operations progress reader; the actionable worst-of health model; structured logging with fail-closed credential-reference scrubbing; trace correlation over recorded ids) and `src/lib/recovery/` (verified online `VACUUM INTO` backup with the append-only manifest + per-row evidence digests; fail-closed restore-into-target with `PRAGMA integrity_check`, migration-checksum match and evidence-integrity verification — a tampered row fails closed, named; idempotent queue replay THROUGH the exported queue/worker classes — same effects/keys, no second effect, UNKNOWN surfaced never retried) plus the drill harness `scripts/test_observability_resilience.mjs` (seven drill groups — family-barrels, backup-restore, worker-restart, replay-recovery, failure-injection, evidence-integrity, telemetry-taxonomy; 30 scenarios / 483 assertions, all green) and `OBSERVABILITY-EVIDENCE.md`. `/api/ready` gains the STRICTLY ADDITIVE `componentHealth` field (the F6 liveness/readiness separation preserved — `/api/health` byte-identical; the enrichment never flips a status code and fails closed to `unknown-data`). Health-signal upgrades recorded for web-api-boundary, durable-command-queue, operational-jobs and external-rail-adapters. The forbidden boundary holds: telemetry is observation only — never financial evidence, never authoritative writes (no `recordEvent(` in the family, no protocol-runtime imports, no network clients); recovery drives the substrate through its exported API only. Declared base records the DEP-007 dispatch base `663b1d4c4e3f1e4b9ebdd2b2758520cb5d8067ed` (DEP-003/004/005 merged — dispatched in parallel with DEP-006 on the same base). The contract delta on the shared surfaces (components.json health-signal upgrades + the validator's DEP-007 section) was Lead-applied at integration per the wave ownership split — DEP-006 owned the shared-governance files during the parallel wave. All four surfaces — components.json, this document, spec/deployment/configuration.md, and the validator — now carry the DEP-007 governed change, and `python3 scripts/validate_deployment.py` exits 0 over the updated contract (1011 checks).
...
- **DEP-006** landed the CI/CD and promotion contract (`spec/deployment/ci-cd.md`) as repository-local deployment-layer automation — no present-set change (all eleven components unchanged; no component added, removed or re-scoped): the gate runner `scripts/run_ci_gates.mjs` (the ordered gate battery — governance, deployment-contract, durable-contract, typecheck, the `scripts/test_*.mjs` harnesses enumerated by glob, build — with the machine-parseable JSON gate table), the promotion tool `scripts/promote.mjs` (subcommands `record` / `migrate` / `verify` / `rollback-plan`: freeze an exact immutable revision — commit SHA, commit subject, tree digest, tracked-file-set content digest — plus the fresh gate table, as an append-only JSONL promotion record in `deploy/promotions/promotion-records.jsonl`; the migration gate against a THROWAWAY copy of the durable database only, verifying the `schema_migrations` applied set + sha256 checksums against the files on disk; record re-verification in a temp worktree at the recorded SHA — recompute digests, re-run the battery, byte-identical gate-table comparison with wall times exempt; rollback-plan emission honoring R1–R5 with UNKNOWN never retried and post-effect cases listed, not executed), the package-snapshot evidence tool `scripts/package_snapshot.mjs` (normalized `build_output_digest` over `.next/standalone` — same tree digest → same build digest across two fresh runs — plus the `image_context_digest` over the docker build context), and the GitHub Actions workflows `.github/workflows/ci.yml` + `promotion.yml` (repo-local gates only; promotion EVIDENCE in CI). Declared base moved to the DEP-006 dispatch base `663b1d4c4e3f1e4b9ebdd2b2758520cb5d8067ed` (DEP-002/003/004/005 all merged — the work order's dependency gate satisfied); owner stays `deployment` for every component; authority only in protocol-layer entries (unchanged); no runtime surface was touched (the tools import `src/lib/durable/db.ts` and `src/lib/environment.ts` as black boxes); the registry gained the top-level `ci_cd` contract object. Production deployment binding (deploying a verified record to production under the separately authorized production configuration) is recorded FUTURE-WORK: every `production_gate` stays authoritative, the promotion tool refuses a resolved `PAYSWAP_ENV=production` context, and unverified production promotion is forbidden. All four surfaces — components.json, this document, spec/deployment/configuration.md, and the validator — were updated together in that one work item, and `python3 scripts/validate_deployment.py` exits 0 over the updated contract.
- **DEP-008** landed the production readiness proof contract (`spec/deployment/production-readiness.md`) as repository-local deployment-layer EVIDENCE — no present-set change (all eleven components unchanged; no component added, removed or re-scoped): the readiness proof harness `scripts/test_production_readiness.mjs` (ten named, counted, fail-closed proof groups — release-identity, protocol-integration, ui-integration, failure-injection, scaling-behavior [SUBSTRATE-level in-sandbox evidence only, never a production-horizontal-scaling claim], config-secret-safety, backup-restore-queue-recovery, unknown-reconciliation, external-effect-safety, rollback-observability — each group checking its assigned work-order stop condition; the harness WRAPS the composed-journey, DEP-007-drill, operations and rail-connectivity harnesses as child processes and composes the DEP-006 promotion tooling: the release-identity group re-derives the recorded content digest in a temp worktree at the recorded SHA with the tool's exact algorithm, re-runs the migration gate via `promote.mjs migrate` inside a throwaway worktree at HEAD, and the rollback groups drive `promote.mjs rollback-plan`), the captured verification transcript `deploy/promotions/DEP-008-READINESS-TRANSCRIPT.md` (the complete evidence: exact command lines, the full group tables, the release record id, the stop-condition audit), and the promotion records for the release revision in `deploy/promotions/promotion-records.jsonl`. The harness's stdout is byte-deterministic (the ci-cd.md §2 battery contract — it is glob-enumerated into the `harnesses` gate; wall-clock timing lives on stderr companion records). A disclosed one-line battery-determinism fix was applied to `scripts/test_observability_resilience.mjs` (a drill note printed the epoch-ms backup id, violating §2 and breaking `promote.mjs verify` for any record at a tree containing it; no assertion, scenario or drill changed). The hard boundary is stated in the contract and in the harness's final line: production readiness is NEVER inferred from sandbox behavior (F8) — the production deployment binding stays FUTURE-WORK and every `production_gate` stays authoritative. The `ci_cd` registry object gained `readiness_harness`, `readiness_transcript`, `readiness_contract` and `readiness_proof_groups`. Declared base moved to the DEP-008 dispatch base `a39cccf312cf55aff6321eb5e0dbbde7936f201b` (DEP-006 + DEP-007 + the product closure candidate UI-010 all merged — the work order's dependency gate satisfied). All four surfaces — components.json, this document, spec/deployment/configuration.md, and the validator — were updated together in this one work item, and `python3 scripts/validate_deployment.py` exits 0 over the updated contract.
- **SYS-001** landed the three-architecture reconciliation (the D-1/D-2/D-3 transport-binding remediation + the reconciliation matrix + the automated checks) — no present-set change (all eleven components unchanged; no component added, removed or re-scoped; no environment or allowlist change): (1) the **D-1 HTTP binding** `src/app/api/protocol/commands/route.ts` — the protocol gateway's command-admission transport over the web boundary: POST forwards one command envelope VERBATIM through `ProtocolGateway.submitCommand` (the A01 admission contract — authority attribution, per-authority body validation, idempotent receipts; responses carry the gateway's typed result vocabulary; typed refusals are HTTP 200 protocol results, malformed JSON and unattributable submissions are transport-typed 400s; GET is the read-only receipt lookup). The route is transport only — NO financial authority, no new externally-configurable name, reachable in every environment class the web-api-boundary serves (development, test-ci, sandbox, staging, production). (2) the **D-2 remediation** `src/lib/protocol/server-composition.ts` — the server-only shared module the routes import, performing the runtime-adapter registration idempotently over the ONE process-global composition (`src/lib/protocol/server-runtime.ts`'s globalThis slot: one runtime, one worker fleet, one set of `var/web-runtime/` SQLite stores per process, shared across every Turbopack module graph — the pre-SYS-001 registration lived only in the instrumentation graph whose register seams were dead-code-eliminated in the built app); twenty-two server-side routes/pages await `ensureProductPortsWired()` before port access. (3) the **D-3 remediation** — `server-runtime.ts`'s `drain()` is now the bounded tick pass (the composed-journey harness's pattern: tick → await in-flight → resume auto-poll → repeat until settled or the 60-pass bound) instead of `worker.stop()` (which halted the worker without executing queued commands). The composition root's gateway-kind alias bindings were extended to the documented registry kinds the binding and the journeys drive (`obligations.clearing.commit`, `queues.item.enqueue`, `capability.register`, `settlement.finality.declare`, `reconciliation.cycle.open` / `statements.collect` / `matching.run` — each alias drives the SAME owning authority command through the SAME single-writer path, the RTN-012 integration-adapter pattern already applied for `obligations.dispute.open` / `queues.item.cancel`). (4) the **reconciliation matrix** `spec/system-reconciliation-matrix.md` + `spec/system-reconciliation-matrix.json` (eight consequential product journeys × the seven reconciliation hops — product action → canonical protocol object/state → owning authority → API/runtime boundary → deployment component → durable state/evidence → user-visible outcome — every cell naming the exact file+export or record type) and the **automated checks** `scripts/test_system_reconciliation.mjs` (authority ownership, no UI-only financial truth, no deployment bypass, explicit environment isolation, UNKNOWN/waiting/recovery preservation — driven over the app's own composition root and the seven ports) plus `scripts/test_transport_binding.mjs` (the built-app proof: the ports resolve the runtime adapters in the RUNNING app, submitted commands EXECUTE, the gateway route admits/typed-refuses per the A01 contract, the presentations stay P5-correct) and `scripts/ts_resolver.mjs` (the harnesses' plain-Node module resolver for the product layer). The deferral-ledger dispositions D-1/D-2/D-3 are RESOLVED with this evidence (`spec/product/closure/deferral-ledger.md`). The web-api-boundary component's `route_surface` gains `/api/protocol/commands` and its `repository_entrypoint` gains the binding's route file and the server-composition module (the governed route-surface change, recorded in components.json together with this document, configuration.md and the validator). The frozen protocol runtime was untouched (`src/lib/protocol-runtime/**`, `src/lib/durable/**`, `src/lib/operations/**`, `src/lib/rail-connectivity/**`, `src/lib/observability/**`, `src/lib/recovery/**` — composed over, never edited); no new npm dependencies. Declared base moved to the SYS-001 dispatch base `46507326265aa5be869d0389f0414d5c7c28f762` (the DEP-008 finalization base — the work order's dependency gate: UI-001..UI-011 + DEP-001..DEP-008 all merged). All four surfaces — components.json, this document, spec/deployment/configuration.md, and the validator — were updated together in this one work item, and `python3 scripts/validate_deployment.py` exits 0 over the updated contract.
- **PC-006** (the console program's deployment work item; design source: docs/superpowers/specs/2026-09-14-payswap-developer-console-design.md §15) landed the console deployment and provider-binding contract — no present-set change (all eleven components unchanged; no component added, removed or re-scoped; no environment or allowlist change; spec/deployment/configuration.md unchanged — the console adds no externally-configurable value): the console ships INSIDE the current single Next.js runtime package (`payswap3` @ `0.1.0`, `next.config.ts` `output: "standalone"` unchanged — packaging.md §1), with NO separate console package and NO topology change. The `web-api-boundary` component's `route_surface` gains the console surface — the 25 static console page routes (the frozen CONSOLE_REGISTRY hrefs of `src/lib/console/registry.ts` minus its one dynamic href `/console/payments/[paymentId]`; the `/console` root is itself a registry href) and the 8 static console API boundary routes (`src/app/api/console/**`, minus the two `[checkoutId]`/`[paymentId]` dynamic routes) delivered by the console program PC-001..PC-005 — and its `repository_entrypoint` gains the console route registry source `src/lib/console/registry.ts` (the single source of truth of the console navigation, cross-checked by the harness). The component registry's route_surface convention is the validator's flat static-route form (entries match `/[A-Za-z0-9._/-]*`), which cannot express path segments; the 3 dynamic routes are therefore recorded ONLY in `deploy/contracts/console-provider-bindings.json`, where the harness proves them against the same filesystem and registry sources. New PC-006 surfaces: `spec/deployment/console-deployment.md` (the console deployment contract — the two-gate separation: repository verification ≠ deployment verification; build/typecheck/CI success is repository proof only, never production proof), `deploy/contracts/console-provider-bindings.json` (the machine-readable provider-binding registry: the closed six-provider set of design §15 — github CONNECTED per the recorded Composio truth [CI-hosting truth only, NOT a console runtime binding and NOT a production deployment binding; evidence reference + verification timestamp + provenance recorded] and vercel/database/queue/cloudflare/observability UNBOUND with explicit null evidence fields and the requirement that live provider evidence must precede any production claim — no fake bindings), `scripts/test_console_deployment.mjs` (the console deployment verification harness — five fail-closed proof groups: package-identity, environment-configuration, route-availability, provider-binding-honesty, repository-deployment-separation; glob-enumerated into the ci-cd.md §2 `harnesses` gate; its verdict line carries `"proves":"repository-facts-only"` and `"production_proof":false`, and a static self-scan proves it performs NO network access) and `spec/console/PC-006-evidence.md` (the delivery evidence). Validator-lock disclosure: `scripts/validate_deployment.py` machine-checks `updated_by == 'SYS-001'` and `base_sha == 46507326265aa5be869d0389f0414d5c7c28f762` (locked at the SYS-001 four-surface change), and PC-006 workers are forbidden from editing the validator — so those provenance fields are intentionally left at their locked values and the PC-006 provenance lives in the components.json `update_note`, this change record, the console binding registry and the evidence document; the Lead applies the validator-side provenance flip (`updated_by` → `PC-006` and, following the DEP precedent, the declared base → the PC-006 dispatch base `09681dda20dfa53d31c81cf96828944763e30b01`) at merge time as the remaining surface of this governed change. The three worker-updated surfaces pass the locked validator as-is: `python3 scripts/validate_deployment.py` exits 0 over the updated contract (1229 checks, up from 1194 at the dispatch base — the +35 checks are exactly the 33 new route_surface entries plus the 2 checks on the new repository_entrypoint path).
- **DEPLOY-D** (the post-closure deployment program; change record: `spec/deployment/serverless-runtime-adaptation.md`) — a governed post-closure deployment ADAPTATION, no present-set change (all eleven components unchanged; no environment or allowlist change; components.json, spec/deployment/configuration.md and this validator untouched — closed WORK/UI/DEP/SYS records untouched): the two runtime WRITE roots now derive from one runtime-state root resolvable through the substrate-level placement directive `PAYSWAP_RUNTIME_DIR` (honored only as a non-empty ABSOLUTE path; unset/empty/relative fail safe to the exact pre-existing `var/` defaults — the environment.ts fail-safe pattern, never a crash), so serverless hosts whose cwd is read-only can point runtime state at a writable instance-local directory (per-instance ephemeral — the recorded non-durable residual; UNKNOWN presentations on fresh instances are the designed honest behavior; no durability claim); `next.config.ts` gains `outputFileTracingIncludes` carrying the runtime-walked `.sql` migration trees (`deploy/migrations` + the seventeen per-domain trees — read-only bundled assets, no runtime state bundled; `output: "standalone"` unchanged); `package.json` pins `engines.node >= 22.13` (node:sqlite unflagged). No protocol/financial/console semantics change; default behavior byte-identical when the directive is unset; `python3 scripts/validate_deployment.py` exits 0 over the unchanged contract (1229 checks).
