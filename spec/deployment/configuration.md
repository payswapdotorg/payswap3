...
**Companions:** `spec/deployment/topology.md` (runtime topology), `spec/deployment/environments.md` (environment classes and fail-closed rules), `spec/deployment/ci-cd.md` (CI/CD and promotion contract, DEP-006), `spec/deployment/observability.md` (observability/resilience/DR contract, DEP-007), `spec/deployment/production-readiness.md` (production readiness proof contract, DEP-008), `deploy/contracts/components.json` (machine-readable registry), `scripts/validate_deployment.py` (verification gate)

## Scope

As of base SHA f934a76 the application exposed exactly **one** externally configurable value: `PAYSWAP_ENV`; DEP-005 added the per-rail rail-connectivity configuration family as the second governed extension of this list; **DEP-006 (the CI/CD and promotion contract, `spec/deployment/ci-cd.md`) added NONE** — the gate runner, the promotion tool and the package-snapshot tool read no configuration of their own: they resolve the environment exclusively through the frozen `src/lib/environment.ts` module (recording the `PAYSWAP_ENV` allowlist context into each promotion record; a resolved `production` context is REFUSED by the promotion tooling — production deployment binding is FUTURE-WORK and every `production_gate` in the registry stays authoritative), and the migration gate locates the durable database through the existing `PAYSWAP_DURABLE_DB` (DEP-003; a tooling input — the gate copies it and never mutates it). **DEP-008 (the production readiness proof contract, `spec/deployment/production-readiness.md`) added NONE** — the readiness proof harness resolves the environment only through the same frozen `src/lib/environment.ts` module (its environment-crossing proof drives the promotion tool's recorded production-context refusal), reads the promotion records and the rail-connectivity resolver through their exported APIs, and adds no configuration surface of its own. This document defines where each value is set, how it is validated, and the secret-boundary rules that govern all configuration. Every externally configurable value for the existing application is listed here; anything not listed is not configurable. The list is closed: new values arrive only with their components' governed work items, together with an update to this document, `deploy/contracts/components.json` and `scripts/validate_deployment.py`.
...
`python3 scripts/validate_deployment.py` (from the repository root) checks that this document states the exact runtime allowlist `sandbox | production` and documents `PAYSWAP_ENV`, in agreement with `src/lib/environment.ts`, `spec/deployment/environments.md` and `deploy/contracts/components.json`. Since DEP-005 it also machine-checks the rail-connectivity configuration agreement: the `PAYSWAP_RAIL_{SCOPE}_{RAIL}_*` pattern names stated here match the `external-rail-adapters` component's `required_configuration` in the registry, and the resolver's variable templates (`src/lib/rail-connectivity/configuration.ts`) agree with both. Since DEP-006 it also machine-checks the CI/CD configuration boundary (see `spec/deployment/ci-cd.md` §10): the CI/CD tooling adds no configurable values — the promotion tooling resolves the environment only through the frozen `src/lib/environment.ts` module and refuses a resolved `production` context. Since DEP-008 it also machine-checks the readiness-proof configuration boundary (see `spec/deployment/production-readiness.md` §6): the readiness harness adds no configurable values and its fail-closed configuration proofs (`validateStartupConfig`'s named production checks, the S1-style secret scan, the environment-crossing refusal) are exercised through the frozen modules' exported APIs. Execution evidence is recorded in the DEP-001 and DEP-005 completion reports; the behavioral evidence (shape validation, fail-closed loading, scope isolation, reference-only credentials) is `scripts/test_rail_connectivity.mjs`; the CI/CD and promotion evidence is `deploy/promotions/` (the append-only promotion records and the DEP-006 rehearsal document); the readiness-proof evidence is `deploy/promotions/DEP-008-READINESS-TRANSCRIPT.md`.
