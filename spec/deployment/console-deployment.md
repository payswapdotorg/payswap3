...
| `vercel` (runtime host) | **CONNECTED** (2026-09-14, live evidence; strengthened 18:08Z) | `spec/deployment/live-deployment-record.md` — the post-closure deployment program's governed flip: dedicated project `payswap3`, Git-built deployment `dpl_ExywTQ2y3XeFjajLD9QpJKmGpp9G` at `c8d8ab1`, `payswap3.vercel.app` serving with browser-verified console navigation (sandbox environment); strengthened after PR #50 merged — org-repo-built production deployment `dpl_8HVzhV5c42Wq4GcPNkdDzDFvnr75` from `payswapdotorg/payswap3@main@ae87ea31` auto-triggered by the Vercel Git integration (GitHub status success), re-verified live (record §8). Live-hosting truth for the sandbox dogfood deployment — NOT a production deployment binding |
...
| Production-like smoke/integration verification against a real deployment | performed twice for the sandbox tier on 2026-09-14 (`spec/deployment/live-deployment-record.md` — real-browser journeys over the live `payswap3.vercel.app` deployment: the fork-built original ≈17:00Z and the org-repo-built re-deployment at 18:08Z §8); the production-promotion verification chain remains FUTURE-WORK above the provider-binding gate |
