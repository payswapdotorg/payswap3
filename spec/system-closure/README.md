...
| `spec/system-closure/closure-record.md` | the Architect closure record DRAFT (family D): program identity, the acceptance mapping (each bullet → matrix rows → mechanical proof → result), the run-time release revision mechanism, the sandbox-environment disclaimer, the honest-handoff ledger summary, the stop-condition audit, and the clearly-marked `Architect sign-off: PENDING (Lead finalize)` block | hand-authored draft; the Architect signs it in the Lead finalize |
